...
* `POST /api/books/:id/outline/plan-next-three`：生成并落盘固定 3 章大纲（`ch_001~ch_003` 的标题与内容大纲）。
* `POST /api/books/:id/chapters/:chapterId/generate`：生成单章正文（流式响应），只允许针对已锁定大纲章节调用。
* `POST /api/books/:id/chapters/batch-generate-three`：三章批量生成编排接口（服务端内部按 `ch_001 -> ch_002 -> ch_003` 串行调用单章生成逻辑，禁止并发）。
